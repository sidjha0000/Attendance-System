...
class attendanceDetails{
    ...
    public function getPresentListOfAClassByAFacOnADate($dbo,$session,$course,$fac,$ondate){
        ...
        where session_id=:session_id and course_id=:course_id and 
        faculty_id=:faculty_id and on_date=:on_date and
        status='YES' ";
        $s=$dbo->conn->prepare($c);
        try{
    ...
    public function getAttendanceReport($dbo,$session,$course,$fac){
        ...
        $facname='';
        $courseNmae='';
        //get the session fac and course names
        ...
        array_push($report,["Course:",$courseNmae]);
        ...
        */
        ...
        for ($i = 0; $i < count($rv); $i++) {
            $rv[$i]['percent'] = 0.00; // Assigning default value
            if ($total > 0) {
                $rv[$i]['percent'] =round(($rv[$i]['attended'] / $total) * 100,2); // Convert to percentage
            }
        }
        array_push($report,["slno","rollno","name","attennded","percent"]);
